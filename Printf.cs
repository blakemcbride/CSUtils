			...
			while ( m.Success )
			{
				...
				case 'u':   // unsigned integer
					w = FormatNumber( ( flagGroupThousands ? "n" : "d" ), flagAlternate,
						fieldLength, int.MinValue, flagLeft2Right,
						false, false,
						paddingCharacter, ToUnsigned( obj ) );
					defaultParamIx++;
					break;
					#endregion
					#region c - character
				case 'c':   // character
					if ( IsNumericType( obj ) )
						w = Convert.ToChar( obj ).ToString();
					else if ( obj is char )
						w = ( (char)obj ).ToString();
					else if ( obj is string && ( (string)obj ).Length > 0 )
						w = ( (string)obj )[0].ToString();
					defaultParamIx++;
					break;
					#endregion
					#region s - string
